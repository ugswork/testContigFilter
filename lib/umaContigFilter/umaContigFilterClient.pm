package umaContigFilter::umaContigFilterClient;
...
umaContigFilter::umaContigFilterClient
...
sub new
{
    ...
    my $self = {
	client => umaContigFilter::umaContigFilterClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is an umaContigFilter.FilterContigsParams
$output is an umaContigFilter.FilterContigsResults
FilterContigsParams is a reference to a hash where the following keys are defined:
	assembly_input_ref has a value which is an umaContigFilter.assembly_ref
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	...
	assembly_output has a value which is an umaContigFilter.assembly_ref
...
# Authentication: required
    ...
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "umaContigFilter.filter_contigs",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub version {
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "umaContigFilter.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for umaContigFilter::umaContigFilterClient\n";
    }
    if ($sMajor == 0) {
        warn "umaContigFilter::umaContigFilterClient version is $svr_version. API subject to change.\n";
    }
}
...
package umaContigFilter::umaContigFilterClient::RpcClient;
